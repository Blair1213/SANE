...
from config import ModelConfig, PROCESSED_DATA_DIR,  ENTITY_VOCAB_TEMPLATE, \
    ...
    DRUG_VOCAB_TEMPLATE,SMILE_HASH,DRUG_SMILE_TEMPLATE,VIRUS_HASH,VIRUS_SEQUENCE_TEMPLATE,ENTITY_HASH
...
def train(train_d,dev_d,test_d,kfold,dataset, neighbor_sample_size, embed_dim, n_depth, l2_weight, lr, optimizer_type,
    ...
    config.entity_hash = np.load(format_filename(PROCESSED_DATA_DIR, ENTITY_HASH),allow_pickle=True)

    config.exp_name = f'TRY_{dataset}_neigh_{neighbor_sample_size}_embed_{embed_dim}_depth_' \
    ...
    auc, acc, f1,aupr, fpr, tpr, r, p = model.score(x=[valid_data[:, :1], valid_data[:, 1:2]], y=valid_data[:, 2:3])
    ...
    model.load_best_model()
    auc, acc, f1, aupr,fpr, tpr, r, p = model.score(x=[test_data[:, :1], test_data[:, 1:2]], y=test_data[:, 2:3])
