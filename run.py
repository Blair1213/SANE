...
def read_entity2id_file(file_path: str, drug_vocab: dict, entity_vocab: dict):
    ...
    with open(file_path, encoding='utf8') as reader:
        ...
        for line in reader:
            ...
            print(row)
            print(len(row))
            drug = row[0:len(row)-1]
            entity = row[len(row)-1]
...
def process_data(dataset: str, neighbor_sample_size: int, K: int, D: int, E: int):
    drug_vocab = {}
    entity_vocab = {}
    relation_vocab = {}
    ...
    print('Logging Info - Saved:', adj_entity_file)
    cross_validation(K, examples, dataset, neighbor_sample_size, D, E)


def cross_validation(K_fold, examples, dataset, neighbor_sample_size, D, E):
    subsets = dict()
    ...
    for t in aggregator_types:
        ...
        temp = {'dataset': dataset, 'aggregator_type': t, 'avg_auc': 0.0, 'avg_acc': 0.0, 'avg_f1': 0.0,
                'avg_aupr': 0.0,'avg_tpr' :[],'avg_fpr':[],'avg_p':[],'avg_r':[]}
        for i in reversed(range(0, K_fold)):
            ...
            if count == 2:
                ...
                temp['avg_tpr'] = [(temp['avg_tpr'][i] + train_log['test_tpr'][i])/2.0 for i in range(0,len(temp['avg_tpr']))]
...
if __name__ == '__main__':
    ...
    import time
    embedding_d = [8,16,32,64,128,256]
    for i in embedding_d:
        process_data('hdvd',5,5,3,E = i)
